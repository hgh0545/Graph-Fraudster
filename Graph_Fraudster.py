...
def opt_attack(model_B, emb_infer_with_noise, target_node, A_B, X_B):
    t = target_node[0]
    emb_A_noise = emb_infer_with_noise[:,:args.out_dim]
    ...
    for i in range(adj.shape[0]):
        modified_adj = A_B.copy().tolil()
        # i = i[0]
        if i==t:
            continue
        else:
            if modified_adj[t, i] != 0:
                modified_adj[t, i] = 0
                modified_adj[i, t] = 0
            else:
                modified_adj[t, i] = 1
        ...
        emb_noise = model_B(X_B, modified_adj)
        dis = F.mse_loss(emb_noise[target_node], emb_B_noise)
        if dis<pert:
            pert = dis.cpu().item()
            adversarial_edge = []
            adversarial_edge.append(np.array([t, i]))
